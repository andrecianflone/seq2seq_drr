...
class EncDec():
  """ Encoder Decoder """
  def __init__(self,hparams, embedding,emb_dim, num_classes=None):
    """
    Args:
    ...
    if hparams.bidirectional == True:
      decoder_num_units = bi_encoder_hidden # double if bidirectional
    else:
  ...
  def get_optimizer(self)
    Opt = locate("tf.train." + self.opt)
    if Opt is None:
      raise ValueError("Invalid optimizer: " + self.opt)
  ...
  def decoder(self, cell, decoder_inputs, seq_len_enc, seq_len_dec,
    ...
    outputs, final_state, final_sequence_lengths= \
              tf.contrib.seq2seq.dynamic_decode(\
                ...
                maximum_iterations=120) # if None, decode till decoder is done
...
class EncDecGen(EncDec):
  """
  EncDec specifically for text generation
  """
  def __init__(self, hparams, embedding, emb_dim):
    ...
    # self.seq_logits = self.sequence_output_logits(
                  # self.decoded_outputs.rnn_output, hparams.dec_out_units, vocab_size)
